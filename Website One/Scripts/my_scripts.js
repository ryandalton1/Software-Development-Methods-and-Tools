...
function viewStudentStats(id, toggle){
	if(toggle == 0){
		//hide the html tag
		//set visibility to hidden and height to 0
		document.getElementById(id).style.visibility = "hidden";
		document.getElementById(id).style.height = "0px";
	}
	else{
		//make the html tag visible
...
function changeColor(color){
	//alert("Funciton called with parameter: "+color);
...
function loadStatsPage(){
	...
	//table.rows[3].cells[3].innerHTML = "fuck this";
	...
	{//Outer for loop iterates over each row
		for(col_counter = 0; col_counter < table.rows[row_counter].cells.length; col_counter++)
		{
			...
			else if(row_counter > 1 && col_counter == 3){
				theirScore = cell_value;
				if(ourScore > theirScore)
					numWins++;
				else
					numLosses++;
			}
			else if(row_counter > 1 && col_counter == 4){
				if(ourScore > theirScore)
					cell_value = "CU Won!"+ourScore+" to "+theirScore;
				else
					cell_value = "CU Lost!"+ourScore+" to "+theirScore;
				table.rows[row_counter].cells[col_counter].innerHTML = cell_value;//Update the actual html of the cell
			}
		}
	}
	table = document.getElementById("wins").innerHTML = numWins;
	table = document.getElementById("losses").innerHTML = numLosses;
...
function loadPlayersPage(){
	...
	for(var player_i=0;player_i < players.length;player_i++){
		selectionText += '<a href="#" onclick="switchPlayers(' + player_i +')">' + players[player_i].name + '</a>';
	}
...
function switchPlayers(playerNum){
	...
	//populate the page with the new data
	avg_p_yards = players[playerNum].pass_yards/players[playerNum].games_played;
	avg_p_yards = Math.round(avg_p_yards * 10)/10;
	avg_r_yards = players[playerNum].rushing_yards/players[playerNum].games_played;
	avg_r_yards = Math.round(avg_r_yards * 10)/10;
	avg_rec_yards = players[playerNum].receiving_yards/players[playerNum].games_played;
	avg_rec_yards = Math.round(avg_rec_yards * 10)/10;
